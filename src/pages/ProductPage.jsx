        ...
        <div className="flex-1 py-8 px-6 flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 items-center">
              ...
              {product?.tags.map((tag) => (
                <div key={tag} className="bg-gray-200 text-xs text-gray-500 py-1 px-3 rounded-sm">
                  {tag}
                </div>
              ))}
            </div>
            <div className="flex gap-1">
              ...
              {
                product?.rating && Array(Math.floor(product?.rating)).fill('').map((_, i) => (
                    <IoIosStar className="fill-amber-400" key={i}/>
                ))
              }
              {
                product?.rating && Array(Math.ceil(5 - product?.rating)).fill('').map((_, i) => (
          ...
          <div className="flex-1 overflow-auto">
            <div className="grid grid-cols-[8rem_1fr] gap-y-4">
              ...
              <p className="text-sm text-gray-700 flex items-center">{product?.warrantyInformation}</p>
              <span className="text-gray-600">Return Policy</span>
              <p className="text-sm text-gray-700 flex items-center">{product?.returnPolicy}</p>
